...
include("../templates/spin_2d_doob_mf.jl")
...
function updateOriginalTransitionRates!(template, idxs::Vector{Int})
    for idx = idxs
        ...
        if template.state[y, x] == 1
            template.originalTransitionRates[idx] = constraint * (1 - template.c)
        else
            template.originalTransitionRates[idx] = constraint * template.c
        end
    end
end
...
function updateOriginalTransitionRates!(template)
    updateOriginalTransitionRates!(template, collect(1:template.size^2))
end
